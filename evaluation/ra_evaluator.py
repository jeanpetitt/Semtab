...
class RA_Evaluator:
    ...
    def _evaluate(self, client_payload):
        ...
        gt_cell_ent = dict()
        gt = pd.read_csv(self.answer_file_path, delimiter=',', names=['tab_id', 'row_id', 'entity'],
                         dtype={'tab_id': str, 'row_id': str, 'entity': str}, keep_default_na=False)
        for index, row in gt.iterrows():
            cell = '%s %s' % (row['tab_id'], row['row_id'])
            gt_cell_ent[cell] = row['entity'].lower().split(' ')

        correct_cells, annotated_cells = set(), set()
        sub = pd.read_csv(submission_file_path, delimiter=',', names=['tab_id', 'row_id', 'entity'],
                          dtype={'tab_id': str, 'row_id': str, 'entity': str}, keep_default_na=False)
